...
2. 双重排版模式：生成的高交互音画同步跳转网页支持“列表模式”与“段落模式”一键切换。默认采用紧凑高利用率的“段落模式”，文本自适应折行并基线对齐时间戳，点击句子或时间戳即刻精准跳转播放。
3. 单页交互界面：主页使用 Bootstrap 5 构建，支持历史记录管理、服务商切换、接口日志实时追加以及 API 密钥 Cookie 选项。
4. 双重凭证安全机制：
   * 支持后端 `.env` 全局配置，对访问者隐藏凭证细节；
   * 支持前端 Web 界面临时输入 API 凭证，前端输入具有绝对优先权，避免环境变量覆盖问题。
5. 智能 SHA256 去重：上传音频时自动计算哈希并匹配，命中缓存时秒级返回结果，避免重复上传和扣费。支持勾选“强制重新识别”。
6. 本地存储：音频文件和转写结果完全保存在部署服务器本地，不依赖第三方对象存储（COS/OSS）。
7. 无第三方依赖：后端基于 Python 纯原生标准库构建，不需要执行 `pip install` 安装第三方包，部署简单。
8. 纯本地化前端资源：前端网页及转写结果页所使用的样式、脚本和字体均使用本地静态资源，无任何外部 CDN（如 Google Fonts、Bootstrap CDN）网络请求依赖，加载极速，原生支持内网或离线部署。
